// -----------------------------------------------------------------------------
// robot_arm.hpp
// Constructs a fixed layout robotic arm (base + 2 arm segments + hand + grippers)
// exposing joint angle fields for animation. Provides setPose/getPose mapping
// from SceneKey to internal joint state.
// -----------------------------------------------------------------------------
#pragma once
#include "model.hpp"
#include <memory>
...
public:
    ...
    // Dimensions used for gripper logic (
    ...
    RobotArm();
    void init();  // Build hierarchy (call only after GL context is valid)
    void updateJoints();
    void draw(GLuint mvpLoc, GLuint modelLoc, const glm::mat4 &viewProj, GLint useTexLoc);


    void setPose(const SceneKey& key); // Apply angles/gripper from SceneKey
    SceneKey getPose() const;          // Export current joint configuration
};