// -----------------------------------------------------------------------------
// robot_arm.cpp : Constructs articulated robotic arm and manages joint updates.
// -----------------------------------------------------------------------------
...
// Build fixed arm structure: base -> lowerArm -> upperArm -> wrist(hand) -> grippers.
void RobotArm::init() {
    model.clear();
    ...
    const float baseTop = baseRadius * baseScaleY; // exact top of base after scaling 
    const float lowerLen = 0.8f;  // lower arm length
    const float upperLen = 0.8f;  // upper arm length 
    const float jointR  = 0.15f;  // visual joint scale 
    const float handH   = 0.25f;  // hand height 
    handHeight = handH;      
...
// Recompute local rotation transforms from public angle fields and gripper open.
...
// Apply a SceneKey's joint angle fields to the robot arm.
void RobotArm::setPose(const SceneKey& key) {
    lowerArmRotX = key.lowerArmPitch;
    lowerArmRotY = key.lowerArmYaw;
    upperArmRotX = key.upperArmPitch;
    upperArmRotY = key.upperArmYaw;
    handRotX     = key.handPitch;
    handRotY     = key.handYaw;
    handRotZ     = key.handRoll;
    gripperOpen  = key.gripperOpen;

    updateJoints();
}


// Capture current joint configuration into a SceneKey (time left at 0).
SceneKey RobotArm::getPose() const {
    SceneKey key;
    key.t = 0; 

    key.lowerArmPitch = lowerArmRotX;
    key.lowerArmYaw   = lowerArmRotY;
    key.upperArmPitch = upperArmRotX;
    key.upperArmYaw   = upperArmRotY;
    key.handPitch     = handRotX;
    key.handYaw       = handRotY;
    key.handRoll      = handRotZ;
    key.gripperOpen   = gripperOpen;
